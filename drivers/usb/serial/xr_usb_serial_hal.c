...
{
        short *data = kmalloc(2, GFP_KERNEL);
		...
		if (result)
			ret = ((*data & 0x8) ? 0: TIOCM_DTR) | ((*data & 0x20) ? 0:TIOCM_RTS ) | ((*data & 0x4) ? 0:TIOCM_DSR) | ((*data & 0x1) ? 0 : TIOCM_RI) | ((*data & 0x2) ? 0:TIOCM_CD) | ((*data & 0x10) ? 0 : TIOCM_CTS); 
		else
			ret = -EFAULT;
		kfree (data);
		return ret;
